...
def RidgeRegressionModel(seed_num = 1, Lab = True, mol = True, cddd = True, ECFP = False, bestCddd = 100, export = False, plot = False, name_submission = "lastSubmission"):
        ...
        if plot:
                plt.figure()
                plt.scatter(y_val, predicted_val, s=2, label="Ridge Regression")
                x = np.linspace(min(y_val), max(y_val), 100) 
                plt.plot(x, x, color='black', linestyle='--')
                plt.title("Ridge Regression - Validation test")
        ...
        if export:
                ...
                saveSubmission(predicted, "RidgeRegression/RidgeRegression_" + name_submission + ".csv")


RidgeRegressionModel(Lab = True, mol = True, cddd = True, ECFP = False, bestCddd = 100, export = True, plot = True, name_submission = "LabMolCddd")